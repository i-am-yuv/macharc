...
export class Canvas {
  ...
  isLastEvent: boolean;
  grabbedNode: HTMLElement | null;
  draggedElement: BlockElement | null;
  draggedTree: Block[];

  constructor({ window, document, node, spacingX = 20, spacingY = 80, initialMaring = 2500 }: any) {
  ...
  appendHtml = (html: string) => (this.node.innerHTML += html);

  appendChild = (...children: (Element | null)[]) => {
    children.forEach((child: any) => this.node.appendChild(child));
  }

  findBlockElement = (id: number) => BlockElement.find(id, { window: this.window });

  import = ({ html, blockarr }: any) => {
    this.html(html);
  ...
  registerDragger = (draggedElement: HTMLElement) => {
    this.draggedElement = BlockElement.fromElement(draggedElement, { window: this.window });
  }

  toggleDragger = (start: boolean, { remove = false } = {}) => {
  ...
  addBlockForElement = (blockElement: BlockElement | null, { parent = -1, childWidth = 0 } = {}) => {
  ...
  findBlock = (id: number, { tree = false } = {}) => (tree ? this.draggedTree : this.blocks).find(block => block.id === id);
  ...
  showIndicator = (show: boolean, block?: Block | undefined) => {
  ...
  setState = (state: { dragX?: number; dragY?: number; currentOffsetLeft?: number; previousOffsetLeft?: number; mouseX?: any; mouseY?: any; }) => {
    return Object.assign(this.state, state);
  }

  getState = (key: string | number) => this.state[key];

  toggleDragging = (dragging: boolean) => {
    this.isDragging = dragging;
  }

  toggleDraggingBlock = (dragging: boolean) => {
    this.isDraggingBlock = dragging;
  }

  toggleRearranging = (rearranging: boolean) => {
    this.isRearranging = rearranging;
  }

  toggleLastEvent = (last: boolean) => {
    this.isLastEvent = last;
  }

  toggleDraggingCanvas = (dragging: boolean) => {
