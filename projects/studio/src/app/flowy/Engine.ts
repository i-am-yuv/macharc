...
export class Engine {
    ...
    public onSnap: (node: HTMLElement, val: boolean, block: Block | undefined) => boolean;

    public import: ({ html, blockarr }: any) => void;
    public output: () => any;
    ...
    constructor(
        ...
        onRelease?: () => void,
        onSnap?: (node: HTMLElement, val: boolean, block: Block | undefined) => boolean) {
        this.onGrab = onGrab!;
    ...
    hasParentClass(node: any, classname: any): any {
    ...
    public snap(block: any) {
        ...
        const { x, y, height }: any = draggedBlock;
    ...
    rearrangeMe() {
        ...
        for (let z = 0; z < parents.length; z++) {
            ...
            childBlocks.forEach(block => {
                ...
                const { x: parentX, y: parentY, height: parentHeight }: any = parent;
                const { x, y, height }: any = this.canvas.blocks.find(({ id }) => id === block.id);
