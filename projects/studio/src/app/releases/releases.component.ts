  ...
  changeMs() {
    if (this.ms.repoId) {
      ...
      this.releasesService
        .getRelease(this.ms.repoId!)
        .then((res: any) => {
          this.releases = res;
          this.gitNotReachable = false;

          this.releasesService
            .getPipelines(this.ms.repoId!)
            .then((res: any) => {
              this.pipelines = res;
              this.suceessPipeline = this.pipelines.filter(
                (t) => t.status === 'success',
              ).length;
              this.failedPipeline = this.pipelines.filter(
                (t) => t.status === 'failed',
              ).length;
              ...
              this.releasesService.getJobs(this.ms.repoId!).then((res: any) => {
                ...
                this.queuedPipeline = this.jobs.filter(
